 ...
 */
 ...
 class RealRequest {
     ...
     * 上传文件
     */
    RealResponse uploadFile(String requestURL, File file,List<File> fileList,Map<String,File> fileMap,String fileKey,String fileType,Map<String,String> paramsMap,Map<String, String> headerMap) {
        HttpURLConnection conn = null;
        try {
            conn = getHttpURLConnection(requestURL,"POST");
            setConnection(conn);
            ...
            if(file != null) {
                writeFile(file, fileKey, fileType, outputStream);//上传文件
            }else if(fileList != null){
                for (File f : fileList){
                    writeFile(f, fileKey, fileType, outputStream);
                }
