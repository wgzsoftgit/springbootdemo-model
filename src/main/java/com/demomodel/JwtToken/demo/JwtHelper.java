package com.demomodel.JwtToken.demo;
...
public class JwtHelper {
    ...
    public static String generateJWT(String userId, String userName, String ...identities) {
        ...
        //将BASE64SECRET常量字符串使用base64解码成字节数组                                                                      -----签名秘钥&&
         ...
         */
        headMap.put("alg", SignatureAlgorithm.HS256.getValue());//alg描述的是签名算法  HS256对称加密
        headMap.put("typ", "JWT");
    ...
    public static String validateLogin(String jsonWebToken) {
        ...
        Claims claims = parseJWT(jsonWebToken); //解析JWT
        if (claims != null) {
            //解密客户编号                                                                  && AES解密，返回String
    ...
    //校验是否被串改
    public static boolean validateLogin1(String jsonWebToken) {
    	boolean mark=false;
        Map<String, Object> retMap = null;
        Claims claims = parseJWT(jsonWebToken); //解析JWT
        if (claims != null) {
    ...
    public static void main(String[] args) {
       ...
       System.out.println("生成JWT字符串"+jsonWebKey);
       Claims claims =  parseJWT(jsonWebKey);// 解析JWT 返回Claims对象
        System.out.println("解析JWT 返回Claims对象"+claims);
       System.out.println("校验JWT是否有效返回json字符串的demo:"+validateLogin(jsonWebKey));// 校验JWT是否有效返回json字符串的demo:
